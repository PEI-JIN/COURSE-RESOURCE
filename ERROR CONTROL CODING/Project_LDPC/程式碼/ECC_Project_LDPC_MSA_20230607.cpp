...
/* Check node operation (CHK) for Min-Sum Algorithm (MSA) */
double CHK(double L1, double L2)
{
    return sgn(L1)*sgn(L2)*min(fabs(L1), fabs(L2));
}
...
int main(void)
{
    ...
    *idum = SEED;   // SEED must be a negative integer
    ...
    for (SNR_dB_Idx = 0; SNR_dB_Idx <= ceil(max_SNR_dB/0.2); SNR_dB_Idx++)
    {
        ...
        if (SNR_dB_Idx == 0)
        {
            fp = fopen("Output_MSA.txt","w");
        }
        else
        {
            fp = fopen("Output_MSA.txt","a");
        }
        ...
        printf("\n!!! Complete Soft-Decision Decoding (MSA) !!!\n");
        printf("SNR(dB): %.1lf\n", SNR_dB);
